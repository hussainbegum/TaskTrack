...
export class AdminDashboardComponent implements OnInit {
  ...
  // Make toastr public for template access
  constructor(
  ...
  if (this.editingUser) {
    ...
    this.adminService.updateUser(this.editingUser.id, { 
      ...
      next: (response) => {
        ...
        // Close modal first
        this.closeUserModal();
        // Then reload data and show success
        this.loadUsers();
        this.toastr.success(`User ${this.userForm.name} updated successfully`, 'Success');
    ...
    console.log('Creating user with data:', this.userForm);
    ...
    this.adminService.createUser({ 
      ...
      next: (response) => {
        ...
        // Then reload data and show success
        this.loadUsers();
        this.toastr.success(`User ${this.userForm.name} created successfully`, 'Success');
        this.addNotification(`New user ${this.userForm.name} was created`);
        this.setView('users');
      },
      error: (error) => {
        console.error('Error creating user:', error);
        this.toastr.error('Failed to create user: ' + (error.message || 'Unknown error'), 'Error');
      }
    });
  }
}
    ...
    if (this.editingTask) {
      this.adminService.updateTask(this.editingTask.id, data).subscribe({
        next: () => {
          this.loadTasks();
          this.closeTaskModal();
          this.toastr.success(`Task "${this.taskForm.title}" updated successfully`, 'Success');
      ...
      this.adminService.createTask(data).subscribe({
        next: () => {
          this.loadTasks();
          this.closeTaskModal();
          this.toastr.success(`Task "${this.taskForm.title}" created successfully`, 'Success');
          this.addNotification(`New task "${this.taskForm.title}" was created`);
          this.setView('tasks');
        },
